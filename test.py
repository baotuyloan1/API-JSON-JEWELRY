...
class Product:
    def __init__(self, id, name, price, image, distance):
        self.id = int(id)
        self.name = name
...
#res = json.dumps([o.dump() for o in products], indent=3)
#print(res)
products_df = pd.DataFrame (products, columns = ['ProductID', 'Discount_Id','ProductName','ProductImage','ProductPrice'])
...
if (len(records) > 3):
    ...
    product_features_df=rating_popular_product.pivot_table(index=['ProductID','ProductName','ProductPrice','ProductImage'],columns='UserID',values='Rating').fillna(0)
    ...
    for i in range(0, len(distances.flatten())):
        if i == 0:
            ...
            arrayProducts.append(Product(id= productId, name= productName, price= productPrice, image = productImage , distance= distances.flatten()[i] ))
